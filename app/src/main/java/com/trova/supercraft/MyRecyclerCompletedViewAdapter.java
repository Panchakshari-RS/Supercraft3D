...
public class MyRecyclerCompletedViewAdapter extends RecyclerView.Adapter<MyRecyclerCompletedViewAdapter.DataObjectHolder> {
    private static String LOG_TAG = "MyRecyclerCompletedViewAdapter";
    ...
    public MyRecyclerCompletedViewAdapter(Context myContext, ArrayList<MyJobsRowItem> myDataset) {
        mContext = myContext;
        mDataset = myDataset;
    }
        ...
        holder.content.post(new Runnable() {
            ...
            public void run() {
                ...
                if((content != null) && (!content.equals("")) && (!content.equals("null")) && (content.length() > 0) && (lineCount > 2)) {
                    if (MyCompletedJobsTabFragment.flag[position] == 0) {
