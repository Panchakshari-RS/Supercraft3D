...
public class MyRecyclerCriticalViewAdapter extends RecyclerView.Adapter<MyRecyclerCriticalViewAdapter.DataObjectHolder> {
    private static String LOG_TAG = "MyRecyclerCriticalViewAdapter";
    ...
    public MyRecyclerCriticalViewAdapter(Context myContext, ArrayList<MyJobsRowItem> myDataset) {
        mContext = myContext;
        mDataset = myDataset;
    }
        ...
        holder.content.post(new Runnable() {
            ...
            public void run() {
                ...
                if((content != null) && (!content.equals("")) && (!content.equals("null")) && (content.length() > 0) && (lineCount > 2)) {
                    if (MyCriticalJobsTabFragment.flag[position] == 0) {
